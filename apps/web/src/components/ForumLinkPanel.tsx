  ...
  return <Panel title="Forum account">
    ...
    {!status ? <button type="button" className="se-btn se-btn--ghost" onClick={refresh}>{error ? 'Retry' : 'Loading connection...'}</button> : status.link ? <>
      ...
      <div className="se-actions-row">
        ...
        <Button type="button" className="se-btn se-btn--ghost" disabledReason={busy ? working : null} onClick={() => setConfirmUnlink(true)}>Unlink forum account</Button>
      </div>
      {confirmUnlink ? <div>
        <p>Remove the public connection between these profiles? Both accounts and their posts stay intact.</p>
        <div className="se-actions-row">
          <Button type="button" className="se-btn se-btn--ghost" disabledReason={busy ? working : null} onClick={() => change(true)}>{busy ? 'Unlinking...' : 'Yes, unlink'}</Button>
          <Button type="button" className="se-btn se-btn--ghost" disabledReason={busy ? working : null} onClick={() => setConfirmUnlink(false)}>Cancel</Button>
        </div>
      </div> : null}
    </> : status.enabled ? <>
      <p>Connect your forum account to show a public link on both profiles, plus your StreetsEmpire badges on the forum and your forum role in the game. You’ll sign into the forum and confirm which account to connect.</p>
      <Button type="button" className="se-btn se-btn--primary se-btn--block" disabledReason={busy ? working : null} onClick={() => change(false)}>{busy ? 'Opening forum...' : 'Link Forum Account'}</Button>
