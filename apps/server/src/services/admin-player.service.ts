  ...
  async inspect(prisma: PrismaClient, roundPlayerId: string, now = new Date()): Promise<AdminPlayerDto> {
    const player = await prisma.roundPlayer.findUnique({
      ...
      include: {
        ...
        activeFavors: { where: { expiresAt: { gt: now } }, orderBy: { expiresAt: 'asc' } },
        armedFavors: { orderBy: [{ armedAt: 'asc' }, { category: 'asc' }] },
        playerQuests: {
          include: { questDefinition: true },
          orderBy: [{ updatedAt: 'desc' }, { attempt: 'desc' }],
        },
    ...
    return {
      ...
      activeFavors: player.activeFavors.map((favor) => ({
        favorKey: favor.favorKey,
        category: favor.category,
        startedAt: favor.startedAt.toISOString(),
        expiresAt: favor.expiresAt.toISOString(),
      })),
      armedFavors: player.armedFavors.map((favor) => ({
        favorKey: favor.favorKey,
        category: favor.category,
        armedAt: favor.armedAt.toISOString(),
      })),
      quests: player.playerQuests.map((quest) => ({
        id: quest.id,
        key: quest.questDefinition.key,
        title: quest.questDefinition.title,
        type: quest.questDefinition.type,
        category: quest.questDefinition.category,
        attempt: quest.attempt,
        status: quest.status,
        isTracked: quest.isTracked,
        isEnabled: quest.questDefinition.isEnabled,
        objectiveProgress: quest.objectiveProgress,
        bonusProgress: quest.bonusProgress,
        chosenBranch: quest.chosenBranch,
        rewardState: quest.rewardState,
        acceptedAt: iso(quest.acceptedAt),
        completedAt: iso(quest.completedAt),
        claimedAt: iso(quest.claimedAt),
        failedAt: iso(quest.failedAt),
        expiresAt: iso(quest.expiresAt),
