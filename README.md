...
24-hour revenge windows in a new strategy ruleset. **0.2.0-E** adds a self-contained onboarding round with seeded local rivals, tooltips, harsher unarmed-thug happiness penalties, armed-thug scouting coverage and public intel privacy, so a solo developer can test recon, raids, wounds, reports and gun-readiness immediately. Older rounds stay pinned to their original rulesets, while the default local seed now makes Game #005 the current 0.2.0-E raid onboarding round. Read the [D implementation notes](docs/COMBAT-0.2.0-D.md),
...
| **0.2.0-E** | seeded local rivals, tooltips, harsher unarmed-thug morale, armed street coverage and hidden opponent intel | **started; local onboarding implemented** |
