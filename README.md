...
**Live version:** `0.4.0-A` &middot; **Ruleset:** `classic-og-v0.4-a` &middot; **Development:** products & vice
...
| **0.4.0-A** | products & vice foundation: ruleset product catalog, hybrid product inventory (crack on its column, other products in rows), Products page | **started** |
...
npm run db:seed           # 8 cities, pinned older rounds, current Game #013 product-foundation round
npm run dev               # api on :3001, web on :5173
```

Open <http://localhost:5173>, register a name, and enter Game #013 - Product Foundation. New players start with cash, thugs, pistols, beer and medicine. Default seeds no bot rivals; for local solo raid testing, run `npm run db:seed:dev-bots` to add active dev bots for cash raids, drug runs, ride theft, lures and drive-bys.
