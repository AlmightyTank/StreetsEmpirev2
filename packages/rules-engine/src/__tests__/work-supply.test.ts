...
describe('planWorkSupply', () => {
  it('burns exactly what crack always burned under the default policy', () => {
    for (const [whores, turns, stock] of [[80, 20, 230], [7, 13, 3], [100, 1, 1000], [3, 144, 0]] as const) {
      const plan = planWorkSupply({ job: 'CASINO', workers: whores, turns, policy: defaultWorkSupplyPolicy(), inventory: { CRACK: stock }, ruleset: classicOgV04B });
  ...
  it('runs 80 Casino workers for 20 turns across Ecstasy, then Cocaine, then dry, in proportion', () => {
    ...
    const plan = planWorkSupply({ job: 'CASINO', workers: 80, turns: 20, policy: casino, inventory: { ECSTASY: 112, COCAINE: 24, CRACK: 0 }, ruleset: tuned });
  ...
  it('never lets a little of a great product supply the whole crew', () => {
    const plan = planWorkSupply({ job: 'CASINO', workers: 100, turns: 10, policy: casino, inventory: { ECSTASY: 1 }, ruleset: tuned });
  ...
  it('strict supply leaves fallback stock alone and runs dry instead', () => {
    const plan = planWorkSupply({ job: 'CASINO', workers: 80, turns: 20, policy: { ...casino, strict: true }, inventory: { ECSTASY: 40, COCAINE: 500, CRACK: 500 }, ruleset: tuned });
  ...
  it('starts substituted when the primary is already gone', () => {
    const plan = planWorkSupply({ job: 'CASINO', workers: 10, turns: 10, policy: casino, inventory: { COCAINE: 100 }, ruleset: tuned });
  ...
  it('treats a trip with nobody to supply as supplied and burns nothing', () => {
    const plan = planWorkSupply({ job: 'CASINO', workers: 0, turns: 20, policy: casino, inventory: {}, ruleset: tuned });
